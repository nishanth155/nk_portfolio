...
export default function About() {
  ...
  const tabContent = {
    1:(
      <div className=''>
        <p className=' font-medium text-2xl text-gray-600 max-md:text-xl'>Dedicated Site Reliability Engineer and Backend Developer with over 5+ years of experience at a product-based company, with a strong foundation in Full stack development. 
    ...
    2:(
      <div className=''>
        <ul  className='flex justify-center gap-8 flex-wrap font-kanit text-2xl text-gray-600'>
    ...
    3:(
      <div className=''>
        {edu_json.map(edu=>
        <div key={edu.id} className=' mb-5 flex-col font-kanit text-xl text-gray-600'>
        ...
        <div>
          {certifications.map(cert=>
            <div key={cert.id} className='mb-5 flex justify-evenly font-kanit text-xl text-gray-600 '>
              <a href={cert.url}>
                <button className='ring-1 ring-slate-300 inset-shadow-sm inset-shadow-slate-400 w-200 h-20 max-md:size-40 rounded-full transition delay-50 duration-200 ease-in-out 
  ...
  return (
    <>
    <div className='flex flex-row m-20 max-md:flex-col max-md:m-5'>
      <div className='flex flex-col max-md:hidden '>
      {tabBar.map(bar => (
          <button 
            key={bar.id} 
            className={`mb-3 font-bold w-90 h-15 text-3xl inset-shadow-sm rounded-2xl
            ${active === bar.id ? 'transition -translate-y-1 scale-110 inset-shadow-gray-50 ring-1 ring-gray-500' : 'bg-white inset-shadow-gray-900 hover:text-4xl'}`}
      ...
      <div className='min-md:not-only:ml-10 place-self-center'>
        {tabContent[active]}
      </div>
    </div>
    </>
